...
**Scope:** Standalone audit plan — separate from mainline feature planning (`.memory/PLAN.md`, which remains the historical record of the fieldball-coach live-checking work).
...
**Action:** Run each tool against `tests/fixtures/fieldball-coach` (and a synthetic bad input). Record: invocation (verify documented command names still resolve post-rename), output shape, exit codes, JSON conformance to check-results spec, failure modes. Verify the `--static` → `--structural` flag rename is consistent between tool and docs.
...
**B1 ✅ (2026-07-16):** Domain overlays shipped, shape ratified (D1 canonical scale ids + domain-native labels — zero schema churn, fixture patterns untouched; D2 detection rules-as-data in `tools/domains/detect.yaml`, agent-applied by survey, explicit override wins; D3 uniform advisory predicate shape + growth rule headers, coordinating OQ#9 without pre-deciding formalization). Files: `{game,web,general}/scales.yaml`, `{web,general}/predicates.yaml` (7 each; game keeps its 13), `detect.yaml`. Skills wired: survey (detect + intake `## Domain` section), formalize (overlay-driven scale step + predicates as Evidence prior art), forces/tensions/model (load overlay vocabulary), resolve (predicates in research sources). Smoke check vs real ~/code projects: tilerush-demo → WEB via express (correctly ignoring game theme — architecture over theme), shader-library → GAME via project.godot/gdshader (rule tuned: depth limit removed, .gd/.gdshader/.tscn signals added after real miss at depth 3), video-boss → GENERAL fallback. Structural validation: all overlay scale ids match the schema enum in canonical order; no dup/incomplete predicates; suite 21/0/1. AGENTS.md layout true; OQ#9 current-state updated. **Web fixture reference for future full run: `~/code/tilerush-demo` (express+ws, 7 ADRs + 3 grills — decision-rich).**
...
- **Q6 (C10 policy):** Large/monorepo → per-area runs + **reconciliation pass**; otherwise full project. Artifacts = live docs, committed to the current project branch. TileRush is area-partitioned.
...
**Field-driven check enhancements ✅ (2026-07-17, tickets 005/006 — from DemoAR run lessons #5):** `include:` glob scoping (string or list, matches file NAMES — the 897-lines-repo-wide fix) and list-valued `target:` (matches unioned; missing entry = loud tool error; space-separated string errors now hint the list form). Suite gains a "Check-Tool Feature Tests" section (5 golden assertions incl. the CK-05 unknown-expect regression) — non-vacuity verified (`extends` present in fixture .gd files; `config_version` absent from client/src). Docs: constraint template check block documents include/list-target/only-in/include_comments; derive skill grep notes rewritten around them; check skill's Backend Prerequisites semgrep row corrected (runner is a stub — `method: semgrep` SKIPs regardless of binary; A4 rule). Machine aligned to repo mise.toml (`mise install` — temurin-21 + smcat). **Suite baseline: 27/0/0.**
**Lessons migration + ticket 007 ✅ (2026-07-17):** `.memory/lessons.md` → `.memory/lessons/` — 12 topical per-lesson files (one-line summary + date + source each) + README index with session log preserving "lessons.md #N" provenance; live refs updated (mise.toml, AGENTS.md layout), historical ticket refs resolve via the session log. **Ticket 007 (operator directive):** ADR 0010 — research-first disposition for ★★ events (refines ADR 0007): mandatory research pass classifies check-defect/spec-noise (propose, no stop), known-and-accepted (log w/ reference, no stop), genuine new decision (HITL WITH research + recommendation); positive evidence required, ambiguity escalates; hard floor (irreversible / security-material-and-novel / contradicts ratified resolution) always blocks; classified-away events reviewed in the span digest. Amended: passup skill step 3 + Does-NOT, conventions ★★ gate rule, AGENTS.md discipline mirror, CONTEXT.md glossary. Field basis: DemoAR `no-tracked-secrets` firing on an icon PNG.
**C3† ✅ (2026-07-17, block 4):** ADR 0009 accepted (`.memory/adr/0009-confidence-evidence-lifecycle.md`) — evidence split by author per grill Q4: machine events → `design/.archwright-evidence.json` (append-only, keyed by `kind:id`, fingerprinted — CK-07 plumbing family; check auto-appends demotion-candidates on ★★/★ FAIL and promotion-candidates on pass streaks/deeper-check passes, citing CK-03 structured output); human ratifications → artifact confidence field + Evidence line citing ledger events (★★ transitions always HITL per ADR 0007/CK-08 precedent; ADR 0010 shapes presentation, not blocking). Report command joins both; passup surfaces ★★ demotion candidates. Rejected: tools-append-to-frontmatter (noisy diffs, self-review smell), derive-from-history (contradicts growth rule 7). Principle: tools write tool-owned files; humans write human-owned files. Ledger IMPLEMENTATION deferred to CK-07 timeframe (R32 fingerprinting still open).
**Ordering adjustment (2026-07-17):** ticket 008 (compile guards into Alloy models — field finding from DemoAR: guards emit as comments, so guard-dependent ★★ invariants sit prose-only) slots BEFORE block 6 (C10): TileRush behavior checks would hit the same wall, and 008's conformance corpus requirement means C10 exercises a proven compiler. Tickets 009/010 (explore, no-urgency) stay post-closeout frontier. Order: block 5 (B10) → ticket 008 → block 6 (C10) → block 7.
...
- TileRush follow-ups live in that repo's tickets, that lane's ownership.
