...
| C10 | Web-domain pipeline run against `~/code/dynamodb-game-demo` (express+ws, 7 ADRs + 3 grills): validates B1 acceptance (web vocabulary, no manual mapping) AND exercises C6 span/digest mechanics (known fog) | P2 | 3h | B1 + C6 — one run closes two known-unknowns |
...
- **Open tickets remaining:** C3 (confidence lifecycle ADR, HITL) · C4 (emitter close-out, decision) · C5 (P3) · C9 (contract validation wiring) · C10 (dynamodb-game-demo web run — closes B1-acceptance + C6-fog).
