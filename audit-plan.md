...
**Ordering adjustment (2026-07-17):** ticket 008 (compile guards into Alloy models — field finding from ExposeAR: guards emit as comments, so guard-dependent ★★ invariants sit prose-only) slots BEFORE block 6 (C10): DynamoRush behavior checks would hit the same wall, and 008's conformance corpus requirement means C10 exercises a proven compiler. Tickets 009/010 (explore, no-urgency) stay post-closeout frontier. Order: block 5 (B10) → ticket 008 → block 6 (C10) → block 7.
