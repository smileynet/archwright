...
| Run fixture tests | `mise run test` (or `tools/run-fixture-tests.sh`) — 26 checks incl. Alloy behavior check (SKIPs with reason if alloy6.jar or java absent; green = 26/0/0) |
...
mise run test                  # verify: 26 passed, 0 failed, 0 skipped
```
...
- After rehydrating the jar, run `mise run test` — the ball-state-lifecycle skip becomes an active behavior check (green = 26/0/0).
