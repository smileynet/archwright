...
| Task | Command |
|------|---------|
| Validate pattern/spec | `python3 tools/archwright-validate.py [--json] <file>...` — validates all kinds incl. contract and discovery (ticket 026: frontmatter schema, ledger entry structure, nothing-invented conservation; approved = errors, proposed = warnings); emits non-fatal `WARN:` lines (e.g., missing `protects_experience`); `--json` emits the CK-03 document shape |
