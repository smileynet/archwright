...
| Publish report | `mise run report-publish -- --project <path> [--name <display>]` — generates report bundle (html+md+json) via check --all + generate.py, commits + pushes `design/report/` in the target project. HTML diagram: ELK Direct SVG (interactive, ~1.4MB inlined); markdown diagram: Mermaid stateDiagram-v2 |
