...
- After ANY merge from upstream — and BEFORE any push that touches `tools/` — `mise run test` (suite green at the final tree; commit messages are not evidence — see `.memory/lessons/port-diffs-not-files.md`). Kiro-global skills are symlinked into this repo (since 5d450bf) and track edits automatically — re-run `mise run deploy-skills` only for newly added skills or when deploying to other tools (claude/codex/agy), whose copies go stale silently.
