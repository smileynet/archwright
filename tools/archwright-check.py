...
def _load_alloy_compiler():
    path = SCRIPT_DIR / "archwright-compile-alloy.py"
    spec = importlib.util.spec_from_file_location("archwright_compile_alloy", path)
...
def _alloy_jar():
    configured = os.environ.get("ARCHWRIGHT_ALLOY_JAR")
    candidates = [
        Path(configured) if configured else None,
        SCRIPT_DIR.parent / ".references" / "org.alloytools.alloy.dist.jar",
        SCRIPT_DIR.parent / ".references" / "alloy6.jar",
    ]
    return next((path for path in candidates if path and path.is_file()), None)


def parse_alloy_results(output, invariants):
    by_name = {_to_field(invariant["id"]): invariant for invariant in invariants}
    results = []
    for line in output.splitlines():
        match = re.search(r"check\s+(\w+).*\b(SAT|UNSAT)\b", line)
        if not match or match.group(1) not in by_name:
            continue
        invariant = by_name[match.group(1)]
        counterexample = match.group(2) == "SAT"
        results.append({
            "invariant": invariant["id"],
            "status": "fail" if counterexample else "pass",
            "message": "Counterexample found" if counterexample else "No counterexample within declared bounds",
            "confidence": invariant.get("confidence", "—"),
            "assurance": "bounded",
            "from_force": invariant.get("from_force"),
            "from_pattern": invariant.get("from_pattern"),
        })
    return results


def parse_counterexamples(receipt):
    counterexamples = {}
    for name, command in receipt.get("commands", {}).items():
        solutions = command.get("solution", [])
        if not solutions:
            continue
        counterexamples[name] = solutions[0].get("instances", [])
...
def check_behavior(data, spec_path):
    """Compile and bounded-check a behavior spec with Alloy."""
    invariants = data.get("invariants", [])
    jar = _alloy_jar()
    if not jar:
        return [{
            "invariant": invariant.get("id", "unknown"),
            "status": "error",
            "message": "Alloy runtime not found; run tools/install-alloy.py",
            "assurance": "none",
        } for invariant in invariants]

    try:
        source = _load_alloy_compiler().generate_alloy(data)
    except ValueError as error:
        return [{"invariant": "compile", "status": "error", "message": str(error), "assurance": "none"}]

    with tempfile.TemporaryDirectory(prefix="archwright-alloy-") as directory:
        directory = Path(directory)
        model = directory / f"{data['id']}.als"
        output = directory / "solutions"
        model.write_text(source, encoding="utf-8")
        try:
            process = subprocess.run(
                ["java", "-Djava.awt.headless=true", "-jar", str(jar), "exec", "-f", "-t", "json", "-o", str(output), str(model)],
                capture_output=True,
                text=True,
                cwd=directory,
                timeout=120,
            ...
            return [{"invariant": "runtime", "status": "error", "message": str(error), "assurance": "none"}]
        receipt_path = output / "receipt.json"
        receipt = json.loads(receipt_path.read_text(encoding="utf-8")) if receipt_path.is_file() else {}
        counterexamples = parse_counterexamples(receipt)
        combined = "\n".join(part for part in (process.stdout, process.stderr) if part)
    if process.returncode != 0:
        return [{"invariant": "runtime", "status": "error", "message": combined.strip(), "assurance": "none"}]
    results = parse_alloy_results(combined, invariants)
    if len(results) != len(invariants):
        return [{"invariant": "runtime", "status": "error", "message": "Alloy output did not report every invariant", "assurance": "none"}]
    model_check = data.get("check", {}).get("model", {})
    for result in results:
        result["scope"] = model_check.get("scope", max(4, len(data.get("states", {}))))
        result["steps"] = model_check.get("steps", max(6, len(data.get("states", {})) * 2))
        name = _to_field(result["invariant"])
        if name in counterexamples:
            result["counterexample"] = counterexamples[name]
    return results
...
def check_conformance(data, spec_path):
    ...
    if check.get("target_status") == "pending":
        return [{
            "invariant": data.get("id", "unknown"),
            "status": "skipped",
            "message": "Check target is pending implementation",
            "assurance": "none",
        }]
        ...
        for r in results:
            if r["status"] in {"fail", "error"}:
                ...
                if r.get("counterexample"):
                    lines.append(f"    counterexample: {json.dumps(r['counterexample'], separators=(',', ':'))}")
...
def check_file(spec_path):
    ...
    if kind == "behavior":
        ...
        results = [{"invariant": data.get("id", "?"), "status": "pass",
                    "message": f"{kind} validation: schema only (no runtime check)"}]
    elif kind == "pattern":
