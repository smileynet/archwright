...
A force inventory (from `archwright-forces`) for one or more areas, and the project domain (from the survey intake outline). Load `../archwright-survey/references/domains/<domain>/scales.yaml` (deployed) — tension names and scale references use the domain's native labels.
