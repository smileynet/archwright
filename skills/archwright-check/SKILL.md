...
- **`code_state`** (ticket 018) — the git identity of the checked tree: `{commit, dirty}`. Emitted in every `--json` document (trace mode included) and stamped onto evidence-ledger events. `dirty: true` = the working tree had uncommitted changes, so the commit does NOT fully identify the checked code — treat such evidence as unverifiable for signoff-grade claims. Git absent or not a repo: `{commit: null, dirty: null, reason}` — a coverage statement on the field, never a crash.
...
- **Entries are created by humans, never by the tool.** Copy the `fingerprints` values from `--json` output into `{"entries": [{"fingerprint": "...", "algo": "aw/v1", "note": "why this debt is accepted"}]}`. `--update-baseline` only ever REMOVES entries that no longer reproduce — and only from FULL-SCOPE runs: it refuses on errored runs, when no baseline exists, and under `--changed-only` (a scoped run only re-proves the affected specs; unaffected specs' entries would read as "resolved" and be wrongly dropped).
...
- `promotion-candidate` — a pass streak reaches `config.promotion_streak` (ledger key, default 5; FAIL resets, error/skip neither counts nor resets), or a ★/— invariant passes a **bounded** (mechanical) check — immediate deeper-tier candidate. ★★ never promotes (top tier). **Streaks count check RUNS, not time or commits** — a tight repair loop (CK-18) re-running a passing spec five times in one session manufactures a candidate from one code state; a candidate is a prompt for human judgment, never proof. (Demotion spam has no equivalent risk: identical re-observations dedup.)
...
- **Commit-binding + staleness (ticket 018, ADR 0009 amendment):** every appended event carries `code_state: {commit, dirty}`; dedup identity is unchanged (a re-observation at a new commit appends nothing — the original binding stands). Staleness is **soft decay by affectedness, judged at consumption**: evidence for spec S recorded at commit C is fresh iff neither S's spec file nor anything under its `check.target` changed since C (CK-19's affectedness predicate with `--base C`); changes elsewhere don't decay it. Dirty-tree evidence counts for day-to-day candidate accumulation but is discounted at ratification. Stale events are presented as historical context when surfacing candidates — never deleted (append-only stands), never hard-invalidated (EDA-style any-change-voids-everything would kill pass streaks under normal commit cadence).
