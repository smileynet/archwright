...
```
**If `merman-cli`/`smcat` are not installed** (they are external tools, not part of archwright — check with `which`): skip PNG rendering, present the Mermaid/smcat source directly, and note that diagrams are unverified. Do not block the phase on missing renderers. To rehydrate them: `npm i -g state-machine-cat` (smcat; PNG output also needs Graphviz `dot`) and `cargo install merman-cli`.
