...
Target: `tests/fixtures/fieldball-coach` (7 code files, post-B7: 3 patterns + 7 specs). All 9 phases walked in compressed form — the fixture's artifacts stand in for phase outputs where they exist; friction recorded where the skill's demands and the artifacts diverge. Tool timings measured; agent-phase durations are approximate (compressed run).
...
| 4 | resolve | HITL gate hit. Treated prior FBC decisions as the human's ratification; confirmation pass only | **Gate friction (C6 evidence):** for a 100%-pre-resolved project the mandatory stop adds a full turnaround with zero new decisions. Exactly the case C6's flow-through class targets — with the ★★-touching caveat |
...
|---|------|--------|----------|
| 1 | Constraint violation blocks `git commit` in FBC | **EXTERNAL** | Proven in FBC (S13); not re-verifiable from this repo. In-repo equivalent proven: violation → exit 1 (B7) |
