...
**Status:** Design complete (design notes were in `.scratch/check-tool-design.md`, since cleaned per scratch policy — this spec is now the authoritative design record). **DoD-5 chain EXECUTED (2026-07-17): CK-03 ✅ CK-04 ✅ CK-05 ✅ CK-09 ✅ CK-10 ✅ CK-21 ✅** — see audit-plan.md §Progress for evidence. CK-05 note: grep backend is pure-Python (upstream Windows-portability decision, 2026-07-17) rather than ripgrep; functionally equivalent (matches → violations[]), B4 acceptance additions (comment-strip, unknown-expect = exit 2) included. Remaining phases (5b baseline CK-06/07/08, 5c ast-grep, 5d SARIF, 5e agent interface) not started. **Executor (grill Q1, 2026-07-17): the audit-plan line of work; DoD-5 critical chain first: CK-03 → CK-04 → CK-05 → CK-09 → CK-10.**
