...
- **Pattern status** — `active` (formalized and checkable) · `fog` (unknown forces / unresolved tension — HITL-blocking) · `gated` (resolution RATIFIED, activation gated on a named future event; requires `gated_on:` naming the unblocking event) · `deprecated` (superseded). Never repurpose fog for a ratified deferral — that corrupts the HITL signal for humans and tooling (ticket 011).
