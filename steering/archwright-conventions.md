...
- Evidence ledger (ADR 0009): in a target project with a `.archwright-evidence.json` (bootstrap: `echo '{}' > design/.archwright-evidence.json`), pass/fail check runs auto-append confidence evidence events (demotion/promotion candidates). When output reports `evidence_ledger.events_appended > 0`, surface the new candidates in the span digest — ratification is human, in the artifact, and ★★ moves always HITL-block. **Every check run also dirties the ledger (streak counters) — fold it into your next commit; a stale dirty ledger blocks `git pull` mid-session (crew-research, 3× on 2026-07-25)**
...
2. **HITL-blocking gates** — always stop and wait for the human:
   ...
   - Any ★★ event that survives the research gate (ADR 0010): before escalating a ★★ violation, research it (prior art, related decision records, contrast pair) — check defects/spec noise get proposed fixes, known-and-accepted items get logged with the reference, and only genuine new decisions (tradeoffs, novel security judgment) block, arriving WITH research + recommended disposition. Hard floor always blocks: irreversible, security-material-and-novel, or contradicting a ratified resolution — where "contradicting" means changing the ratified INTENT. An **intent-preserving achievability correction** (ratified text physically unachievable or factually wrong; fix preserves the ratified intent) may be agent-applied WITHOUT blocking, with mandatory mechanics per ADR 0014: one-line intent statement, provenance citing ADR 0014 + evidence, listed in the span digest. Three-leg test before applying (state intent without the broken mechanism / unachievability is citable fact / ratifier would recognize the fix) — any doubt → escalate. ★★ assignment beyond what resolve ratified and demotion proposals still block unconditionally
...
3. **Research before generating.** 2+ independent sources or a spike before writing the new instance. Dispatch parallel research subagents covering prior art, best practices, and related ecosystems (operator-set shape, tickets 049/050) — findings that shaped the instance get a durable synthesis (`.memory/research-*.md` or the ADR); raw subagent reports stay ephemeral. Spike output IS the conformance scenario.
